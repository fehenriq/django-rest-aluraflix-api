...
class AuthenticationUserTestCase(APITestCase):
    ...
    def test_autenticacao_de_user_com_username_incorreto(self):
        """Teste que verifica autenticação de username incorreto"""
        user = authenticate(username='c3p0', password='12345')
        self.assertFalse((user is not None) and user.is_authenticated)
        
    def test_autenticacao_de_user_com_password_incorreta(self):
        """Teste que verifica autenticação de password incorreta"""
        user = authenticate(username='c3po', password='1234')
        self.assertFalse((user is not None) and user.is_authenticated)
